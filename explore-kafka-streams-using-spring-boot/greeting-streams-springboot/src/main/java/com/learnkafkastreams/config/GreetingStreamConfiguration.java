...
//        var kafkaStreamProperties = kafkaProperties.getStreams().buildProperties();
//        kafkaStreamProperties.put(StreamsConfig.DEFAULT_DESERIALIZATION_EXCEPTION_HANDLER_CLASS_CONFIG,
//                RecoveringDeserializationExceptionHandler.class);
//        kafkaStreamProperties.put(RecoveringDeserializationExceptionHandler.KSTREAM_DESERIALIZATION_RECOVERER
//                , recoverer());
...
//    }
    ...
    private ConsumerRecordRecoverer recoverer() {
        return ((consumerRecord, e) -> {
            log.error("Exception is : {}, Failed Record : {}", e.getMessage(), consumerRecord.value());
        });
    }
    }
